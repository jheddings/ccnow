...
export interface PwdData {
  name: string;
  path: string;   // full prefix path (without basename or trailing /), e.g. '/Users/test'
  smart: string;  // smart-truncated prefix (without basename or trailing /), e.g. '~/t'
}
...
function smartPrefix(cwd: string): string {
  ...
  const parts = p.split('/');
  // 2 segments (e.g. /tmp) — just the leading slash or ~
  if (parts.length <= 2) return parts[0] || '/';

  const first = parts[0]; // '' for absolute, '~' for home-relative
  const middle = parts.slice(1, -1);

  // 3 segments (e.g. ~/Projects/ccnow) — keep prefix as-is, no trailing /
